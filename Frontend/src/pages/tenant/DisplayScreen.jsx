...
export default function DisplayScreen() {
  const { branchId, tenantType } = useParams();
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      console.log("Display screen connected to live queue updates");
      socket.emit("joinBranch", branchId);
      // Fetch latest state on connect/reconnect to ensure no missed events
      fetchDisplayData();
    });

    socket.on("queueUpdated", (updateData) => {
      console.log("Queue updated", updateData);
      // Play a sound when a token is called
      if (updateData.action === "callNextToken" || updateData.action === "iotNextToken" || updateData.action === "skipAndCallNextToken" || updateData.action === "recallToken") {
        try {
        ...
        <div className="flex-[2] flex flex-col rounded-3xl bg-slate-800 shadow-2xl overflow-hidden border border-slate-700">
          ...
          <div className="flex-1 p-8 grid grid-cols-2 gap-8 auto-rows-max overflow-y-auto">
            {called.length === 0 ? (
              ...
              called.map((token) => (
                <div key={token._id} className="flex flex-col items-center justify-center rounded-2xl bg-slate-700 p-8 shadow-inner border border-slate-600 transform transition-transform hover:scale-[1.02]">
                  <div className="text-2xl text-slate-400 font-semibold mb-2 uppercase tracking-wide">
                    {token.serviceName}
                  </div>
                  <div className="text-7xl font-black text-amber-400 tracking-tighter mb-4">
                    {token.tokenNumber}
                  </div>
                  <div className="text-2xl text-white font-medium bg-slate-900 px-6 py-2 rounded-full">
                    Counter {token.counterId ? "Assigned" : "Active"}
                  </div>
                </div>
              ))
            )}
          ...
          <div className="flex-1 p-6 flex flex-col gap-4 overflow-y-auto">
            {waiting.length === 0 ? (
              ...
              waiting.map((token, index) => (
                <div key={token._id} className="flex items-center justify-between rounded-xl bg-slate-700 p-6 border-l-4 border-blue-500 shadow-sm">
